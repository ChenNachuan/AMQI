...
% 恢复 ctex 默认字体配置（请使用 xelatex 或 lualatex 编译）
...
% 统一标题字体与正文（使用楷体，与 \kaishu 保持一致）
\setbeamerfont{frametitle}{family=\kaishu,size=\large,series=\bfseries}
...
\begin{frame}{XGBoost 模型简介}
    XGBoost（极端梯度提升算法）是一种基于 Boosting 框架，串行训练 $K$ 棵分类回归树 (CART)的集成学习算法。
...
\begin{frame}{XGBoost策略构建}
    ...
    \begin{figure}
        ...
        \begin{tikzpicture}[xscale=0.8, yscale=0.5]
            ...
            \fill[blue!30] (0,0.2) rectangle (4,0.8) node[midway, black] {\tiny Train (24m)};
            \fill[red!30] (4,0.2) rectangle (5,0.8) node[midway, black] {\tiny Test};
            \fill[blue!30] (1,-0.8) rectangle (5,-0.2) node[midway, black] {\tiny Train (24m)};
...
\begin{frame}{核心绩效指标}
    ...
    \begin{table}[]
        ...
        \textbf{Ann. Return} & 23.16\% & 33.49\% & 22.17\% & 35.27\% & 14.37\% \\ 
        \textbf{Sharpe Ratio} & 0.7815 & 1.0662 & 0.6757 & 1.0623 & 0.3975 \\ 
        \textbf{Max Drawdown} & -64.49\% & -62.61\% & -74.68\% & -65.04\% & -69.27\% \\ 
        \textbf{Win Rate} & 60.70\% & 62.81\% & 60.70\% & 63.16\% & 47.37\% \\ \hline
        \end{tabular}
        }
    \end{table}
    2. 模型特性分析
...
\subsection{CNE-5模型和行业检验}
\begin{frame}{CNE-5 模型和行业检验}
    \begin{itemize}
        \item \textbf{回归方程} (每月横截面回归):
        \begin{equation*}
            R_{i, t+1} = \gamma_{ML} \cdot \hat{y}_{i,t} + \sum_{k=1}^{10} \gamma_{Style, k} \cdot F_{Style, k} + \sum_{j=1}^{31} \gamma_{Ind, j} \cdot D_{Ind, j} + \epsilon_{i,t}
        \end{equation*}
        \item 控制变量: 10个 Barra 因子 + 31个申万一级行业变量
    \end{itemize}

    \begin{table}[]
        \centering
        \renewcommand{\arraystretch}{1.2}
        \setlength{\tabcolsep}{8pt} % 增加列间距
        \resizebox{0.95\textwidth}{!}{%
        \begin{tabular}{l|cc|cc}
        \hline
        \multirow{2}{*}{\textbf{Factor}} & \multicolumn{2}{c|}{Random Forest (RF)} & \multicolumn{2}{c}{XGBoost (XGB)} \\ \cline{2-5} 
         & Return ($\gamma$) & t-stat & Return ($\gamma$) & t-stat \\ \hline
    	\textbf{ML Score (Pre\_ret)} & \textbf{22.71\%} & \textbf{20.99} & \textbf{12.73\%} & \textbf{15.30} \\ \hline
        Size & -1.18\% & -7.79 & -1.61\% & -10.01 \\ 
        Non-linear Size & -0.72\% & -7.84 & -1.05\% & -10.87 \\ 
        Liquidity & -0.44\% & -4.10 & -0.59\% & -5.48 \\ 
        Earnings Yield & 0.21\% & 3.38 & 0.20\% & 3.22 \\ 
        Momentum & 0.18\% & 2.34 & 0.14\% & 1.92 \\ \hline
        \end{tabular}%
        }
    \end{table}
...
\begin{frame}{收益归因分析 : 方法论与累计收益分解}
    \textbf{1. 核心归因公式}
    \begin{equation*}
        R_{Port} = \text{Market Exp} \cdot f_{Mkt} + \sum \text{Style Exp} \cdot f_{Style} + \sum \text{Ind Exp} \cdot f_{Ind} + \text{Alpha}
    \end{equation*}
    \begin{itemize}
        \item \textbf{Alpha (残差项)}: 代表模型依靠选股能力获得的\textbf{纯超额收益}。
    \end{itemize}

    \begin{figure}
            \centering
            \includegraphics[width=\textwidth, height=0.45\textheight, keepaspectratio]{pic/combined_attribution.pdf}
    \end{figure}
    \vspace{-0.3cm}
    \footnotesize
    \begin{itemize}
        \item Alpha 贡献: 图中顶部的\textcolor{red!80!black}{\textbf{红色区域}}持续显著增长，有强大的选股能力。
        \item \textbf{风格影响}: \textcolor{blue!70!black}{\textbf{蓝色区域}}有一定的正向贡献，说明策略部分收益来自于风格偏好（如小市值）。
    \end{itemize}
            % 这里可以放一个小的示意图或者留白，主要靠文字解释逻辑
    
\end{frame}

\begin{frame}{收益归因分析: 风格暴露}
    \begin{figure}
            \centering
            \includegraphics[width=0.92\textwidth, keepaspectratio]{pic/combined_style_exposures.pdf}
    \end{figure}
        
    \textbf{风格差异}: 两个策略在Size 和Non\_linear\_size 上暴露均相反，说明RF捕捉了A股长期的小市值红利，而XGBoost则利用非线性交互逻辑在大市值股票中寻找超额收益。
...
\begin{frame}{参考文献}
    % 若正文中未显式引用，使用 nocite 列出 bib 中所有条目
    \nocite{*}
...
\begin{frame}
    ...
    \end{center}
    \vfill % 推到底部
    \footquote{\footnotesize 本项目代码已开源于 \textcolor{blue}{\url{https://github.com/nachuanchen/AMQI}}}\\
\end{frame}
